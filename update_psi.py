...
def update_psi(Psi, Mask, Observed_image, Latent_image, gamma=1, lambda1=0.002, lambda2=10):
    ...
    # TODO fix it: currently psi_x, psi_y are arrays (3 channels)
    for i in range(height):
        for j in range(width):
            for k in range(n_channel):
                ...
                Psi[0][i][j][k] = update_single_psi(psi_x, mask, dx_obs, dx_lat, gamma, lambda1, lambda2)
                Psi[1][i][j][k] = update_single_psi(psi_y, mask, dy_obs, dy_lat, gamma, lambda1, lambda2)
